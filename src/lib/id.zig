...
fn renameTask(arena: std.mem.Allocator, dir: std.fs.Dir, task_name: []const u8) !void {
    const new_task_name = try new(arena);
    try dir.rename(task_name, new_task_name);
    var description_buffer_old: [64]u8 = undefined;
    var description_buffer_new: [64]u8 = undefined;
    const old_name = std.fmt.bufPrint(&description_buffer_old, "{s}.md", .{task_name}) catch unreachable;
    const new_name = std.fmt.bufPrint(&description_buffer_new, "{s}.md", .{new_task_name}) catch unreachable;
    const description_file_old = try std.fs.path.join(arena, &.{ new_task_name, old_name });
    const description_file_new = try std.fs.path.join(arena, &.{ new_task_name, new_name });
    try dir.rename(description_file_old, description_file_new);
    std.debug.print("Renamed {s} to {s}", .{ task_name, new_task_name });
}
...
pub fn getGilaDir(gpa: std.mem.Allocator) ?struct { []const u8, std.fs.Dir } {
    ...
    const gila_dir = std.fs.openDirAbsolute(gila_path, .{}) catch {
        return null;
    };
...
pub fn searchForGilaDir(gpa: std.mem.Allocator) ?[]const u8 {
    const pwd: []const u8 = std.process.getCwdAlloc(gpa) catch {
        return null;
    };
    var current_dir: []const u8 = pwd;

    outter_loop: for (0..128) |_| {
        var dir = std.fs.openDirAbsolute(current_dir, .{ .iterate = true }) catch {
            return null;
        };
        defer dir.close();

        var dir_walker = dir.iterateAssumeFirstIteration();
        while (dir_walker.next() catch {
            return null;
        }) |entry| {
...
fn migrateFromDateTimeToGecko(gpa: std.mem.Allocator) !void {
    _, var gila_dir = getGilaDir(gpa) orelse return;
    defer gila_dir.close();

    const fixed_buffer: []u8 = gpa.alloc(u8, std.fs.max_path_bytes) catch unreachable;
    var path_arena = std.heap.FixedBufferAllocator.init(fixed_buffer);

    inline for (std.meta.fields(gila.Status)) |field| {
        var dir_n = gila_dir.openDir(field.name, .{ .iterate = true }) catch null;
        if (dir_n) |*dir| {
            defer dir.close();
            ...
            while (try dir_walker.next()) |entry| {
                if (entry.kind == .directory) {
                    if (gila.TaskId.isValidFormat(entry.name)) {
                        try renameTask(path_arena.allocator(), dir.*, entry.name);
                        path_arena.reset();
                    }
                }
            }
        } else {
            std.log.debug("Failed to open {s}", .{field.name});
        }
    }
}
...
pub fn main() !void {
    ...
    try migrateFromDateTimeToGecko(arena.allocator());
}
